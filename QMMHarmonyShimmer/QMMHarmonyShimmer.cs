...
namespace QMMLoader.QMMHarmonyShimmer
{
    ...
    public static class QMMHarmonyShimmer
    {
        ...
        {
            if (ShimQModInstaller())
            {
                ShimQMods();
            }
        }

        public static bool ShimQModInstaller()
        {
            if (!File.Exists(QModInstallerPath))
            {
                Logger.LogError("QModInstaller.dll not found! Shimming aborted.");
                return false;
            }

            Logger.LogDebug("Shimming QModManager...");
            ...
            try
            {
                using (var stream = new MemoryStream(File.ReadAllBytes(QModInstallerPath)))
                {
                    ...
                    {   // Unmerge the assembly
                        shimmed = true;
                        Logger.LogDebug($"Unmerging {Path.GetFileNameWithoutExtension(QModInstallerPath)}");
                    ...
                    {   // Shim Harmony
                        shimmed = true;
                        Logger.LogDebug($"Shimming {Path.GetFileNameWithoutExtension(QModInstallerPath)}");
                    ...
                    if (shimmed)
                    {
                        var backupPath = $"{QModInstallerPath}.bak";
                        Logger.LogInfo($"Backing up {Path.GetFileNameWithoutExtension(QModInstallerPath)} to {backupPath}. Original path: {QModInstallerPath}");
                        File.Copy(QModInstallerPath, backupPath, true);

                        TypeDefinitionIsDefinitionPatch.definitionsAreReferences = true;
                        assemblyDefinition.Write(QModInstallerPath);
            ...
            {
                Logger.LogError($"Failed to shim {Path.GetFileName(QModInstallerPath)}");
                Logger.LogError(e);
                return false;
            }
