...
namespace QModManager
{
    public static partial class QModAPI
    {
        ...
        public class MessageReceiverAttribute : Attribute
        {
            ...
            {
                Sender = GetMod(senderAssembly, true, true) ?? throw new ArgumentException("The provided assembly is not a mod assembly!", nameof(senderAssembly));
            }
            public MessageReceiverAttribute(string senderID)
            {
                // Should we not want to throw an exception in case the mod is not present?
                Sender = GetMod(senderID, true, true) ?? throw new ArgumentException($"No mod matching the provided ID \"{senderID}\" was found!", nameof(senderID));
            }
        }
