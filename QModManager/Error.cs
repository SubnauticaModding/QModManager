...
namespace QModManager
{
    ...
    {
        internal static void ShowError(string error, Action onLeftButtonPressed = null, Action onRightButtonPressed = null, string leftButtonText = "See Log", string rightButtonText = "Close")
        {
            uGUI_SceneConfirmation confirmation = uGUI.main.confirmation;

            if (onLeftButtonPressed == null) onLeftButtonPressed = () => { Process.Start(Path.Combine(QModPatcher.QModBaseDir, "../Subnautica_Data/output_log.txt")); };
            if (onRightButtonPressed == null) onRightButtonPressed = () => { Console.Write(""); };
